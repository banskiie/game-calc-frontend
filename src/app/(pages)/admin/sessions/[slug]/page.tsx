...
const END_GAME = gql`
  mutation UpdateGame(
    ...
    updateGame(
      input: { _id: $id, end: $end, status: $status, session: $session }
    ) {
      _id
      end
      status
...
const Page = () => {
  ...
  const [fetchUsers, { data: usersData, refetch: refetchUsers }] =
    useLazyQuery(FETCH_USERS);
  ...
  const [isSessionSettingsModalOpen, setIsSessionSettingsModalOpen] =
    useState(false);
  ...
  const [fetchCourts] = useLazyQuery(FETCH_COURT, {
    onCompleted: (data) => {
      ...
      setAvailableCourts(
        data.fetchCourts.filter(
          (court: any) => !currentCourtIds.includes(court._id)
        )
      );
    },
  });
  ...
  const handleAddPlayers = async () => {
    const playersToAdd = [
      ...new Set([...tempSelectedPlayers, ...selectedPlayers]),
    ];
  ...
  useEffect(() => {
    if (data?.fetchSession?.court?.length > 0 && activeTab === "all") {
      ...
      const defaultTab =
        woodCourt?._id || data.fetchSession.court[0]?._id || "all";
  ...
  useEffect(() => {
    if (isPlayerSelectModalOpen) {
      ...
      const availablePlayersIds =
        session?.availablePlayers?.map((player: any) => player._id) || [];
  ...
  const gamesByCourt = sessionCourts.reduce((acc: any, court: any) => {
    acc[court._id] = {
      name: court.name,
      games:
        gameData?.fetchGamesBySession?.filter(
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <span className="font-bold text-md">
                ...
                {game?.end ? format(new Date(game.end), "hh:mm a") : "Ongoing"}
              </span>
              {game?.end && (
  ...
  const getNewPlayersToAdd = () => {
    const currentSessionPlayerIds =
      session?.availablePlayers?.map((p: any) => p._id) || [];
    const allSelected = [
      ...new Set([...selectedPlayers, ...tempSelectedPlayers]),
    ];
    return allSelected.filter((id) => !currentSessionPlayerIds.includes(id));
  };

  const newPlayersCount = getNewPlayersToAdd().length;
      ...
      <CardContent>
        <div className="flex flex-col items-center">
          ...
          <p className="text-sm text-gray-600">
            It seems like there are no Games Scheduled/Created. Try checking
            back later!
          </p>
        </div>
      </CardContent>
        ...
        <CardContent className="flex flex-col items-center text-center pb-2">
          ...
          {allGames.games.length > 0 && (
            <span className="block text-muted-foreground font-semibold text-lg">
              ...
              {(() => {
                const endedGames = allGames.games.filter(
                  (game: any) => game.end
                );
                if (endedGames.length === 0) return "Ongoing";

                const latestEnd = endedGames.reduce(
                  (latest: any, game: any) =>
            ...
            <div className="flex-1 overflow-y-auto py-4 scrollbar-hide">
              ...
              <TabsContent value="courts" className="h-full">
                <CourtMultiSelect
                  ...
                  selectedCourts={
                    selectedCourtToAdd ? [selectedCourtToAdd] : []
                  }
                  onSelectCourt={(courtId) => {
                    setSelectedCourtToAdd(
                      courtId === selectedCourtToAdd ? null : courtId
                    );
                  }}
                />
              </TabsContent>
